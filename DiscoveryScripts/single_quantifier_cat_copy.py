    ...
    if len(run_files)<1:
      ...
      os.system('salmon quant -i '+index+' -l A '+input_name+' -p 1 --validateMappings -o '+output_name)
        ...
        for z in np.arange(0, len(run_files)):
            if "R1" in run_files[z]:
                input_name+=' '+run_files[z]
        for z in np.arange(0, len(run_files)):
            if "R2" in run_files[z]:
                print('make reverse: '+run_files[z])
                os.system('gzip -d '+run_files[z])
                new_name = run_files[z].replace("R2","R1.2")
                print("new name: "+new_name)
                os.system("fastx_reverse_complement -z -i "+run_files[z]+" -o "+new_name)
        for z in np.arange(0, len(run_files)):
            if "R1" in run_files[z] and ".gz" in run_files[z]:
                os.system('gzip -d'+run_files[z])
            if "R1" not in run_files[z]:
                os.system('rm '+run_files[z])
